...
import (
	...
	loggingv3 "github.com/rancher/types/apis/logging.cattle.io/v3"
...
type LoggingContext struct {
	...
	Logging loggingv3.Interface
	RBAC    rbacv1.Interface
	Core    corev1.Interface
}

func (c *LoggingContext) controllers() []controller.Starter {
	return []controller.Starter{
		c.Logging,
	}
}

func NewLoggingContext(config rest.Config) (*LoggingContext, error) {
	var err error
...
func (w *LoggingContext) Start(ctx context.Context) error {
	logrus.Info("Starting logging controllers")
	return controller.SyncThenStart(ctx, 5, w.controllers()...)
}

func (w *LoggingContext) StartAndWait(ctx context.Context) error {
	ctx = signal.SigTermCancelContext(ctx)
	w.Start(ctx)
	<-ctx.Done()
	return ctx.Err()
